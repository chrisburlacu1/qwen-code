...
export class QwenConnectionHandler {
  ...
  async connect(
    ...
    const authType = config.get<string>('qwenCode.authType', 'ollama');

    // Build extra CLI arguments (only essential parameters)
    const extraArgs: string[] = [];
    if (authType) {
      extraArgs.push(`--auth-type=${authType}`);
    }
    ...
    if (!sessionRestored) {
      ...
      try {
        ...
        await this.newSessionWithRetry(connection, workingDir, 3, authType);
