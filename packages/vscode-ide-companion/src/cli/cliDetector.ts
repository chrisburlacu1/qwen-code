...
/**
 * Detects if Sosh CLI is installed and accessible
 */
export class CliDetector {
  ...
  /**
   * Checks if the Sosh CLI is installed
  ...
  static async detectQwenCli(
    ...
    try {
      ...
      try {
        ...
        const detectionCommand =
          process.platform === 'win32'
            ? `${whichCommand} sosh`
            : 'source ~/.nvm/nvm.sh 2>/dev/null && (nvm use default 2>/dev/null || nvm use node 2>/dev/null || nvm use 2>/dev/null); which sosh';
        ...
        try {
          ...
          const versionCommand =
            process.platform === 'win32'
              ? 'sosh --version'
              : 'source ~/.nvm/nvm.sh 2>/dev/null && (nvm use default 2>/dev/null || nvm use node 2>/dev/null || nvm use 2>/dev/null); sosh --version';
        ...
        // CLI not found
        let error = `Sosh CLI not found in PATH. Please install it using: npm install -g @qwen-code/qwen-code@latest`;
      ...
      let userFriendlyError = `Failed to detect Sosh CLI: ${errorMessage}`;
  ...
  static getInstallationInstructions(): {
    ...
    return {
      title: 'Sosh CLI is not installed',
      steps: [
        'Install via npm:',
