...
export function determineNodePathForCli(
  ...
  const nodePathPatterns = [
    // NVM pattern: .../bin/sosh -> .../bin/node
    normalizedPath.replace(/\/bin\/sosh(\.cmd|\.exe)?$/, '/bin/node'),

    // Manual installation pattern: .../sosh -> .../node
    normalizedPath.replace(/\/sosh(\.cmd|\.exe)?$/, '/node'),
  ];
  ...
  const cliDir = normalizedPath.substring(0, normalizedPath.lastIndexOf('/'));
  const potentialNodePaths = [
    `${cliDir}/node`,
    `${cliDir}/server/bin/node`,
    `${cliDir}/bin/node`,
  ];
