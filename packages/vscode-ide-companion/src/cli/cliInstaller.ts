 ...
 * Responsible for detecting, installing, and prompting for Sosh CLI
 */
export class CliInstaller {
  ...
  static async checkInstallation(
    ...
    try {
      ...
      if (!result.isInstalled) {
        console.log('[CliInstaller] Sosh CLI not detected:', result.error);
      } else {
        console.log(
          '[CliInstaller] Sosh CLI detected:',
          result.cliPath,
          result.version,
        );
      }
  ...
  static async promptInstallation(): Promise<void> {
    const selection = await vscode.window.showWarningMessage(
      'Sosh CLI is not installed. You can browse conversation history, but cannot send new messages.',
      'Install Now',
      'View Documentation',
      'Remind Me Later',
    );
      ...
      await vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          try {
            ...
            if (detection.isInstalled) {
              vscode.window
                .showInformationMessage(
                  `✅ Sosh CLI installed successfully! Version: ${detection.version}`,
                  'Reload Window',
                )
                .then((selection) => {
            ...
            let userFriendlyMessage = `Failed to install Sosh CLI: ${errorMessage}`;
